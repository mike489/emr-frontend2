...
import { Formik, Form, Field, ErrorMessage, type FieldProps, getIn, useFormikContext } from 'formik';
...
const ExaminationForm: React.FC<ExaminationFormProps> = ({ consultationId }) => {
  ...
  const handleSubmit = async (values: ExaminationData, { setSubmitting }: any) => {
    ...
    // Prepare examination_data with proper field names and null handling
    const examination_data = {
      ...values,
      primary_complaint: values.primary_complaint || '', // This field exists in form
    ...
    try {
      const response = await PatientService.createExamination(payload);
      ...
      if (err.response) {
        ...
        toast.error('Request setup failed: ' + err.message);
      }
    } finally {
      setSubmitting(false);
    }
    ...
    >
      {({ values, touched, errors }) => {
        ...
        const FloatingActionButtons = () => {
          const { submitForm, isSubmitting: formikSubmitting } = useFormikContext<ExaminationData>();
            ...
            <Fade in>
              ...
              >
                ...
                <Button
                  ...
                  disabled={formikSubmitting}
                  size="large"
                  startIcon={formikSubmitting ? <CircularProgress size={20} /> : <SaveIcon />}
                  sx={{ minWidth: 200 }}
                >
                  {formikSubmitting ? 'Saving...' : 'Save Examination'}
                </Button>
              </Box>
            </Fade>
          );
          ...
          <Form>
            ...
            >
              ...
              >
                ...
                <AccordionDetails>
                  {/* Primary Complaint – Quill - FIXED FIELD NAME */}
                  <Grid size={{ xs: 12 }}>
                      ...
                      {({ field, form }: FieldProps) => (
                        <Paper elevation={1} sx={{ borderRadius: 1, overflow: 'hidden' }}>
                          <ReactQuill
                            theme="snow"
                            value={field.value || ''}
                            onChange={(v) => form.setFieldValue('primary_complaint', v)}
                            modules={quillModules}
                            formats={quillFormats}
                            style={{ height: 200, border: 'none' }}
                            placeholder="Provide detailed information about the primary complaint..."
                          />
                          {getIn(form.touched, 'primary_complaint') && getIn(form.errors, 'primary_complaint') && (
                            <Typography color="error" variant="caption" sx={{ ml: 2, mt: 0.5, display: 'block' }}>
                              {getIn(form.errors, 'primary_complaint')}
                            </Typography>
                          )}
                        </Paper>
                      )}
              ...
              {/* -------------------------------------------------
                  IOP
              ------------------------------------------------- */}
              <Accordion expanded={expandedSection === 'iop'} onChange={handleSectionChange('iop')} sx={{ mb: 2 }}>
                <AccordionSummary expandIcon={<ExpandMoreIcon />}>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                    <BiotechIcon color="primary" />
                        ...
                        {({ field, form }: FieldProps) => (
                          <div>
                            <ReactQuill
                              theme="snow"
                              value={field.value || ''}
                              onChange={(v) => form.setFieldValue('primary_diagnosis', v)}
                              modules={quillModules}
                              formats={quillFormats}
