        ...
        {({ values, isSubmitting, touched, errors }) => (
          <Form>
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, flexGrow: 1 }}>
                  ...
                  <Box>
                    <Typography variant="h6" fontWeight="bold">
                      {initialValues.primary_complaint
                        ? 'Edit Examination'
                        : 'New Examination Record'}
                    </Typography>
                    {visitData && (
                      <Typography variant="body2" sx={{ opacity: 0.9 }}>
                        {visitData.patientName} •{' '}
                        {new Date(visitData.visitDate).toLocaleDateString()} • {visitData.visitType}
                      </Typography>
                    )}
                  </Box>
                </Box>
                <IconButton edge="end" color="inherit" onClick={onClose} disabled={isSubmitting}>
                  <CloseIcon />
                </IconButton>
              </Toolbar>
            </AppBar>
                ...
                >
                  ...
                  <AccordionDetails>
                    ...
                    <Grid container spacing={3} sx={{ mt: 1 }}>
                      {(['systemic_conditions', 'allergies', 'family_history'] as const).map(
                        field => {
                          const label = field
                            .replace(/_/g, ' ')
            ...
            <DialogActions sx={{ p: 2, borderTop: 1, borderColor: 'divider' }}>
              <Button onClick={onClose} disabled={isSubmitting} startIcon={<CloseIcon />}>
                Cancel
              </Button>
              <Button
                type="submit"
