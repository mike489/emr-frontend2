...
const TestNew: React.FC<TestNewProps> = ({
  open,
  onClose,
  patientId,
  patientName,
  ...
  const hasPendingTests = () => {
    ...
    return tests.some(test => !test.result && test.is_payment_completed);
  };
        ...
        <Box sx={{ mt: 2 }}>
          ...
          {success && (
            <Alert severity="success" sx={{ mb: 2 }}>
              Operational results submitted successfully! Files will be organized in patient_tests
              directory.
            </Alert>
          )}
                  ...
                  <AccordionSummary expandIcon={<ExpandMore />}>
                    <Box sx={{ width: '100%' }}>
                      <Box display="flex" justifyContent="space-between" alignItems="center">
                        ...
                        <Box display="flex" alignItems="center" gap={1}>
                          <Chip
                            label={`${getAllTests().filter(t => !t.result && t.is_payment_completed).length} pending`}
                            size="small"
                            color="warning"
                            variant="outlined"
                          />
