  ...
  useEffect(() => {
    if (open && followUpData) {
      ...
      const fetchPatient = async () => {
        ...
        try {
          const res = await PatientService.getList({ department: 'Triage'});
          setCurrentPatient(res.data?.data.data.find((p: Patient) => p.id === followUpData.patient_id) || null);
        } catch (err) {
      ...
      <DialogTitle>
        ...
        {patientLoading ? (
          ...
          <Typography variant="subtitle1" color="primary" sx={{ mt: 1, fontWeight: 500 }}>
            Patient Name: {currentPatient.full_name}
          </Typography>
        ) : (
      ...
      <DialogContent dividers>
        <Box sx={{ pt: 2 }}>
          <Grid container spacing={3}>
            ...
            {/* Rich Text Fields */}
            <Grid size={12} sx={{ mt: 4 , height:'100%'}}>
