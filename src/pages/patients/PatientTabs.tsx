...
import { Tabs, Tab, Box, Paper, CircularProgress, Alert } from '@mui/material';
    ...
    <Box display="flex" flexDirection="column" width="100%" sx={{ px: 2, pb: 4, mt: -16 }}>
      ...
      >
        ...
        >
          ...
          <Tab label="Radiology" value="radiology" />
        </Tabs>
        ...
        <Box flexGrow={1} p={3} bgcolor="background.paper">
          ...
          {currentTab === 'follow-up' && isFollowUpVisit && (
            <FollowUpListTable patientId={patient.id} consultantId={patient.constultation_id} />
          )}

          {/* Medical Certificate */}
          {currentTab === 'certificate' && <MedicalCertificatesIndex patient={patient} />}
          ...
          {currentTab === 'laboratory' && (
            <Box textAlign="center" py={8}>
              <Fallbacks title="No Data Found" description=" Laboratory Module Coming Soon" />
            </Box>
          )}
          {/* Radiology */}
          {currentTab === 'radiology' && (
            <Box textAlign="center" py={8}>
              <Fallbacks title="No Data Found" description=" Radiology Module Coming Soon" />
            </Box>
          )}
        </Box>
      </Paper>
